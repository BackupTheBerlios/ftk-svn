
import ftk.core.error.FtkError;

/**
 * base exception for serialization package
 * 
 * @author     Patrick Müller <elias{edd}adaptiveinstance{dod}com>
 * @copyright  Patrick Müller 2006
 * @license    http://www.opensource.org/licenses/lgpl-license.php  LGPL
 * @see        http://adaptiveinstance.com
 * @see        http://developer.berlios.de/projects/ftk/
 * @since      0.10
 * @version    SVN: $Id$
 * @version    Release: @package_version@
 */

class ftk.serialization.SerializationError extends FtkError
{
	
	public function SerializationError(msg:String, n:Number)
	{
		super(msg, n);
	}

}
